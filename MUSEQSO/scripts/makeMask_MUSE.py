...
def determine_mask(wc,line, redshift):
    ...
    sigma_v = {"Lya": 3.e3, "CIV": 3e3, "SiIV": 3e3, "CII": 3e3, "NV": 3e3, "OI": 3e3, "OVI": 3e3, "HeII": 3e3, "continuum":3e3}  # km/s turbulent velocity
    ...
    return lmin,lmax
def update_mask(subdir,tab):
    line = "Lya"
    name=os.path.basename(subdir)
    element=tab[tab["Quasar"]==name]
    adp_file = ctools.find_adp_fits_file(subdir)
    if adp_file:
        hdu = fits.open(adp_file+".fits")[1]
        hdr = hdu.header
        redshift = element["z_Lyaneb"]
        ...
        runcubetool_script = os.path.join(subdir, "runcubtool_QSO.sh")
...
if __name__ == "__main__":
    ...
    all_directories=ctools.find_directories_from_ascii(qsos,root_directory,filters=filters)
